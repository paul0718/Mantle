...
namespace Opertoon.Panoply {
	public class PanoplyThumbnailMenu : MonoBehaviour {
		...
		void Start () {
			_foregroundCamera = transform.Find ("Foreground Camera")?.GetComponent<Camera>();
		...
		public bool ScreenPointIsInsideMenu(Vector2 point) {
			if (_canvas.renderMode == RenderMode.ScreenSpaceCamera) {
				...
				// camera param not needed if render mode is Screen Space - Overlay
		...
		private void InitializeMenu()
		{
			...
			if (eventSystem == null) {
				...
				eventSystem.name = "PanoplyEventSystem";
			}
