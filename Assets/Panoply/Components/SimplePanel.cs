...
namespace Opertoon.Panoply {
	...
	public class SimplePanel: MonoBehaviour
	{
		...
		public void Start() {
			...
			List<float> homePositions = new List<float> ();

			float maxHomePositionX = 0;
			Panel [] panels = FindObjectsOfType (typeof (Panel)) as Panel [];
			foreach (Panel panel in panels) {
				if (panel != this) {
					homePositions.Add (Mathf.Round (panel.homePosition.x));
			...
			if (homePositions.Contains (Mathf.Round (homePosition.x))) {
				...
				if (scene != null) {
					homePosition.x = maxHomePositionX + scene.homePositionXSpacing;
				}
			}
